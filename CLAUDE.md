...
- **Cloudflare Pages:** assume the build runs in CI. Don't rely on local-only env vars without documenting them in `.env.example`. The Pages project, custom domains, DNS, and the `www → apex` Redirect Rule are codified in OpenTofu under `infra/` (see ADR 0006 in `~/GitHub/notes/adr/frosty/`); changes go through PR review and CI applies on merge to `main`.
