...
<video src={demo} width="1548" height="1080" autoplay muted loop playsinline preload="auto" aria-label="Retrofolio backtest UI walkthrough" />
...
Three AI features reuse the backtest pipeline's infrastructure rather than standing up a parallel stack.

The first is **explanation**. After a run finishes you can ask for a plain-English read of what the numbers mean: why a 0.90 Sharpe with a 37% drawdown is a different animal than the same Sharpe with a 12% one. The text streams token-by-token over the same SSE plumbing the backtests use, and the finished explanation is cached in Postgres (`backtest_explanations`) so re-opening a run doesn't regenerate it.

The second is a **conversational agent**: a chat drawer for follow-up questions about portfolio theory and the runs you've done. Each conversation is a persisted session (`agent_sessions` + `agent_messages`), a single turn streams as resumable SSE (drop the connection and a `Last-Event-ID` header replays from where the stream left off), and only one turn can be in flight per session at a time. Long threads get a lazily-maintained context summary so the prompt doesn't grow without bound, and idle sessions are pruned on the same Celery beat schedule that sweeps everything else.

The third is **semantic search** over your backtest history. Each finished run becomes a short document (portfolio, window, the four headline metrics, and its explanation if it has one), embedded with Voyage and stored as a `vector(1024)` in Postgres via pgvector. A free-text query like "low-volatility dividend portfolios" is embedded the same way and ranked by cosine similarity over an HNSW approximate-nearest-neighbor index, and a "find similar runs" button reuses a run's stored vector with no API call at all. Embeddings backfill lazily on the same Celery workers under a daily cost cap, so the feature costs nothing until someone actually searches.

All three sit behind feature flags, so the AI surface ships and rolls back on its own cadence without touching the backtester's deploy path.

## What I'd do next

The roadmap below predates the AI layer; the quant features are still the next thing I want to build out. Three things on the list.
