...
Reads and writes split cleanly. The backtest write endpoint (`POST /backtest`) always goes through the queue. Every read path (history list, run detail, CSV export, job status) goes straight to Postgres or Redis without touching a worker. Persistent state is still small: six tables (`users`, `backtest_runs`, `revoked_tokens`, plus `agent_sessions`, `agent_messages`, and `backtest_explanations` for the AI layer below), with the heavy backtest output stored as JSON columns on the run row. Redis is the Celery broker, the result backend, and the market-data cache, but nothing in Redis is the system of record.
...
## An AI layer on top of the backtester

Two AI features reuse the backtest pipeline's infrastructure rather than standing up a parallel stack.
...
Both features sit behind feature flags, so the AI surface ships and rolls back on its own cadence without touching the backtester's deploy path.
