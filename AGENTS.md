...
- **Diagrams.** Pre-render with `mmdc` (`npx -p @mermaid-js/mermaid-cli mmdc -i diagram.mmd -o src/assets/<name>.svg -b transparent`), commit the SVG to `src/assets/`, and embed it via an asset import in MDX. Wrap in `<figure class="diagram">` to escape the 60ch reading column. Don't install `rehype-mermaid` for a single diagram — it renders SVG in a real headless browser, pulling in Playwright (~90MB) and ~10s of fresh-build time. Revisit the build-time renderer only once the site has 3+ diagrams to maintain.

## Cloudflare zone (`infra/`)

`infra/` manages the whole `dhfrost.com` zone, not just this site. SSL mode, Always Use HTTPS, and minimum TLS (`zone-settings.tf`) and the CAA records (`dns.tf`) apply to every subdomain, including `retrofolio.dhfrost.com`, whose origin serves a Cloudflare Origin Certificate. Before changing any of them, check each active subdomain's origin TLS; a mismatch under strict mode returns 526. The edge also strips some origin response headers on subdomains (`Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Opener-Policy`); re-adding them needs a zone-level `http_response_headers_transform` ruleset here.

## Verify the live site

`pnpm build` can't catch broken links, CSP violations, missing favicons or OG images, or redirect mistakes. After a deploy that touches routing, layout, headers, or metadata, load `https://dhfrost.com` in a fresh browser session: check the pages you changed, that links and CTAs resolve, and that the console is clean (CSP, 404s). For visual bugs, screenshot the rendered page before theorizing about causes.
